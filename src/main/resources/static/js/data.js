...
function fetchData(page) {
    ...
    const temperature = document.getElementById("searchTemp").value;
    const humidity = document.getElementById("searchHumidity").value;
    const brightness = document.getElementById("searchLight").value;

    if (startTime != "") startTime = convertToISO(startTime);
    if (endTime != "") endTime = convertToISO(endTime);
    const url = `/data-sensor?page=${page - 1}&size=${pageSize}&sortBy=${currentSortField}&direction=${sortDirection}&temperature=${temperature}&humidity=${humidity}&brightness=${brightness}&start=${startTime}&end=${endTime}`;
...
function clearSearch() {
    ...
    document.getElementById('searchHumidity').value = '';
    document.getElementById('searchLight').value = '';
}
